...
carica_configurazione() {
    if [ -f "$CONFIG_FILE" ]; then
        # Legge le righe del file. Se mancano dei valori, usa i default di fabbrica
...
# MODIFICA CONFIGURAZIONE (-ec)
modifica_config_permanente() {
    echo "=== MODIFICA CONFIGURAZIONE PERMANENTEMENTE ==="
...
eval "for file in $input_files; do 
    if [ -f \"\$file\" ]; then
        ...
        # Esecuzione dinamica basata sulle variabili impostate o passate
